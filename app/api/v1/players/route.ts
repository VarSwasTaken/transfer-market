...
export async function GET(request: Request) {
  ...
  const nationalityId = parsePositiveInt(url.searchParams.get('nationalityId')) ?? undefined;
...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body.firstName !== 'string' || typeof body.lastName !== 'string' || typeof body.birthDate !== 'string' || typeof body.position !== 'string' || (typeof body.marketValue !== 'string' && typeof body.marketValue !== 'number') || typeof body.nationalityId !== 'number') {
      return Response.json({ ok: false, error: 'Invalid payload for player creation.' }, { status: 400 });
    }

    if (!isValidShirtNumber(body.shirtNumber)) {
      return Response.json({ ok: false, error: 'shirtNumber must be an integer between 1 and 99.' }, { status: 400 });
    }

    const normalizedPosition = body.position.toUpperCase();
    if (!(normalizedPosition in Position)) {
      return Response.json({ ok: false, error: 'Invalid position value.' }, { status: 400 });
    }
    ...
    const created = await createPlayer({
      ...
      preferredFoot: body.preferredFoot === 'LEFT' || body.preferredFoot === 'RIGHT' || body.preferredFoot === 'BOTH' ? body.preferredFoot : undefined,
